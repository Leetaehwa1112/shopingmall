...
import { getCard, formatKRW, formatKRWFull, ALL_CARDS } from '@/api/cards'
...
export default function ProductDetailPage() {
  ...
  const related = ALL_CARDS.filter((c) => c.id !== card.id && c.category === card.category).slice(0, 4)
  ...
  const handleBuyNow = () => {
    add(card)
    toast({ type: 'success', title: '장바구니에 추가', message: card.nameKo })
  }
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-10">
      ...
      <div className="text-xs font-bold text-mute mb-8 flex items-center gap-2">
        ...
        <Link to={card.type === 'auction' ? '/auctions' : '/products'} className="hover:text-ink">
          {card.type === 'auction' ? '경매' : '카탈로그'}
        </Link>
        <Icon name="arrow" size={10} strokeWidth={2} className="opacity-50" />
        <span className="text-ink">{card.nameKo}</span>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-12">
        ...
        <div className="space-y-4">
          {/* Main view */}
          <div className="surface-soft p-10 flex justify-center items-center min-h-[560px] relative overflow-hidden"
            style={{ background: `radial-gradient(ellipse at center, ${card.accent}10 0%, transparent 70%)` }}>
          ...
          <div className="grid grid-cols-4 gap-2">
            {[
              ...
              >
                <div className="absolute inset-0 flex items-center justify-center scale-[0.42] origin-center">
                  {v.render}
                </div>
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <span className="pixel-label text-mute">Lot #{card.id.slice(0, 6).toUpperCase()}</span>
          </div>
          ...
          <div>
            ...
            <h1 className="font-display font-bold text-5xl lg:text-6xl text-ink tracking-tight leading-none">
              {card.nameKo}
            </h1>
            <div className="text-xl italic text-mute mt-2">{card.name}</div>
          </div>
          ...
          {card.type === 'auction' ? (
            <div className="surface-soft p-6 elev-2 space-y-5">
              <div className="flex justify-between items-end">
                <div>
                  ...
                  <div className="text-xs text-mute font-mono mt-2">{formatKRWFull(card.currentBid)}</div>
                </div>
              ...
              {/* LCD countdown — the only "game" element here */}
              ...
              {/* Quick bid */}
              <div>
                <div className="text-[10px] font-bold text-mute tracking-[0.18em] uppercase mb-2">빠른 입찰</div>
                <div className="grid grid-cols-3 gap-2">
                  {[
                    ...
                    <button key={mult} onClick={() => quickBid(mult)}
                      className="px-2 py-2.5 bg-bone-2 hover:bg-line border border-line rounded-lg transition-all">
                      <div className="text-xs font-bold text-ink">{label}</div>
                      <div className="text-xs text-dex font-mono font-bold mt-0.5">{formatKRW((card.currentBid + 1000000) * mult)}</div>
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div className="flex gap-2">
                <input
                  type="text" value={bidAmount} onChange={(e) => setBidAmount(e.target.value)}
                  placeholder={`최소 ${formatKRWFull(minBid)}`}
                  className="flex-1 bg-bone-2 border border-line rounded-lg px-4 py-3 font-mono text-sm text-ink focus:border-ink focus:bg-paper outline-none transition-colors"
                />
                <Button variant="accent" size="md" onClick={handleBid}>입찰</Button>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2 pt-4 border-t border-line">
                <Button variant="secondary" size="sm" onClick={() => wishlist.toggle(card.id)}>
                  <Icon name="star" size={14} strokeWidth={1.8} style={{ fill: wishlist.has(card.id) ? '#f5b800' : 'none' }} />
                  {wishlist.has(card.id) ? '관심 등록됨' : '관심 등록'}
                </Button>
            ...
            <div className="surface-soft p-6 elev-2 space-y-5">
              ...
              <Button variant="secondary" size="sm" className="w-full" onClick={() => wishlist.toggle(card.id)}>
                <Icon name="star" size={14} strokeWidth={1.8} style={{ fill: wishlist.has(card.id) ? '#f5b800' : 'none' }} />
                {wishlist.has(card.id) ? '관심 등록됨' : '관심 등록'}
              </Button>
            </div>
          ...
          {/* Shipping banner — price-based */}
          <ShippingBanner price={card.price || card.currentBid} />

          {/* Trust mini */}
          <div className="grid grid-cols-2 gap-2 text-xs">
            {[
              ['shield', '정품 100% 보증', 'blue'],
              ['trophy', 'PSA 공식 인증', 'yellow'],
              ['package', '보안 운송 + 보험', 'red'],
              ['lock', '에스크로 결제', 'green'],
            ].map(([icon, text, c]) => (
      ...
      <div className="mt-16">
        <div className="flex gap-1 mb-8 border-b border-line overflow-x-auto">
          {[
            ['description', '카드 정보'],
            ['history',     card.type === 'auction' ? '입찰 내역' : '출처'],
            ['sales',       '시세 추이'],
            ['shipping',    '배송'],
          ].map(([id, label]) => (
          ...
          {tab === 'description' && (
            <div className="space-y-4">
              ...
              <div className="grid sm:grid-cols-2 gap-2 mt-6 max-w-2xl">
                ...
                <KV k="등급" v={`${card.grade.company} ${card.grade.score}`} />
                <KV k="등급 라벨" v={card.grade.label} />
                <KV k="인증서 번호" v={card.grade.cert} />
              </div>
            </div>
          )}

          {tab === 'history' && card.type === 'auction' && (
      ...
      {related.length > 0 && (
        <div className="mt-20">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {related.map((c) => <CardTile key={c.id} card={c} />)}
          </div>
        </div>
      )}
    </div>
    ...
    <div className="grid grid-cols-2 gap-1 w-full h-full p-1 bg-bone-2 rounded-md">
      {['0% 0%', '100% 0%', '0% 100%', '100% 100%'].map((bg, i) => (
        <div key={i} className="bg-white rounded"
          style={{ backgroundImage: `url(${card.image})`, backgroundSize: '320%', backgroundPosition: bg }} />
      ))}
    </div>
  )
}
